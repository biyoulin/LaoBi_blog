...
def make_paginator(object, page, num=5):
    paginator = Paginator(object, num)
    try:
...
def pagination_data(paginator, page):
    ...
    if page_number == 1:
        ...
        # 说明最右边的页码号和最后一页的页码号之间还有其他页码，因此需要显示省略号，通过 left_has_more 来指示
        if right[-1] < total_pages - 1:
            right_has_more = True

        # 如果最右边的页码号比最后一页的页码号小，说明当前页右边的连续页码号中不包含最后一页的页码
        # 所以需要显示最后一页的页码号，通过 list 来指示
        ...
        # 此时只要获取当前页左边的连续页码号
        # 比如分页页码列表是[1,2,3,4],那么获取的额就是 left = [2,3]
        # 这里只获取了当前页码后连续两个页码，你可以更改这个数字以获取更多页码
        ...
        # 用户请求的既不是最后一页，也不是 第 1 页，则需要获取当前页左右两边的连续页码号
        # 这里只获取了当前页码前后连续两个页码，你可以更改这个数字以获取更多空间
        left = page_range[(page_number - 3) if (page_number - 3) > 0 else 0:page_number - 1]
